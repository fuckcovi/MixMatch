...
<div class="page-main-style">
		...
		<c:if test="${count > 0}">
			<c:forEach var="league" items="${list}">
				<tr>
					...
					<td><input type="button" value="상세보기" class="btn" onclick="location.href='leagueDetail.do?l_seq=${league.l_seq}'"></td>
				</tr>
			</c:forEach>	
		</c:if>
	</table>
	...
	<c:if test="${!empty user_id}">
		<div>
			<input type="button" value="리그등록" class="btn" onclick="location.href='leagueInsert.do'">
		</div>
	</c:if>
</div>