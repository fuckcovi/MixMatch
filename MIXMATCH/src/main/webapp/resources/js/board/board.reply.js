$(document).ready(function(){
	...
	//댓글 목록
	function selectData(pageNum,gb_seq){
		currentPage = pageNum;
		...
		$.ajax({
			type:'post',
			data:{pageNum:pageNum,gb_seq:gb_seq},
			url:'listReply.do',
			dataType:'json',
			cache:false,
			timeout:30000,
			success:function(data){
				...
				if(count < 0 || list == null){
					...
					$(list).each(function(index,item){
						var output = '';
						output += '<div class="item" id="'+item.gbre_no+'">';
						output += '  <h4>' + item.id + '</h4>';
						output += '  <div class="sub-item">';
						output += '    <p>' + item.gbre_content + '</p>';
						output += '    <div>' + item.gbre_date;
						
						if($('#user_id').val() && $('#user_id').val() == item.id){
							//로그인한 id가 댓글 작성자 id와 같은 경우
							output += ' <input type="button" value="수정" data-num="'+item.gbre_no+'" data-id="'+item.id+'" class="modify-button">';
							output += ' <input type="button" value="삭제" data-num="'+item.gbre_no+'" data-id="'+item.id+'" class="delete-button">';
						}else{
	...
	$(document).on('click','.modify-button',function(){
		...
		var modifyUI =  '<form id="mre_form">';
		    modifyUI += ' <input type="hidden" name="gbre_no" id="mre_no" value="'+num+'">';
		    modifyUI += ' <input type="hidden" name="id" id="muser_id" value="'+id+'">';
		    modifyUI += ' <textarea rows="3" cols="50" name="gbre_content" id="mre_content" class="rep-content">'+content+'</textarea>';
